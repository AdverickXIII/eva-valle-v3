...
def plot_shannon_barras(df: pd.DataFrame, min_area: float = 1000) -> go.Figure:
    diversidad = calculate_shannon_diversity(df)
    if diversidad.empty:
        return apply_theme(go.Figure(), "Indice de Diversificacion Shannon-Wiener por Municipio")
    diversidad = diversidad[diversidad["area_total"]>min_area].sort_values("shannon_wiener")
    fig = go.Figure()
    fig.add_trace(go.Bar(x=diversidad["shannon_wiener"],y=diversidad["municipio"],
        orientation="h",marker_color=PALETTE[0],
        text=[f"{v:.2f}" for v in diversidad["shannon_wiener"]],textposition="outside"))
