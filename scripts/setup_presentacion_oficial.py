"""Paquete oficial: Ficha Tecnica (formato BID) + Presentacion Ejecutiva 10 laminas."""
from pathlib import Path

MOD = '''"""Ficha tecnica oficial (estandar BID) + presentacion ejecutiva de EVA Valle."""
...
Path("core/reports/presentacion_oficial.py").write_text(MOD, encoding="utf-8")
print("[OK] core/reports/presentacion_oficial.py creado")

# --- Generar los 2 PDFs en outputs/ para abrirlos YA ---
import sys
...
(out / "ficha_tecnica_EVA_Valle.pdf").write_bytes(build_ficha_tecnica_pdf())
(out / "presentacion_ejecutiva_EVA_Valle.pdf").write_bytes(build_presentacion_pdf())
print("[OK] outputs/ficha_tecnica_EVA_Valle.pdf")
print("[OK] outputs/presentacion_ejecutiva_EVA_Valle.pdf")

# --- Botones en la pagina Reportes ---
rp = Path("ui/pages/10_Reportes.py")
c = rp.read_text(encoding="utf-8")
if "presentacion_oficial" not in c:
    idx = c.find("placeholder")
    if idx != -1:
        inicio = c.rfind("\n", 0, idx) + 1
        bloque = (
        ...
        c = c[:inicio] + bloque + c[inicio:]
        rp.write_text(c, encoding="utf-8")
        print("[OK] Botones agregados en Reportes")
    else:
        print("[AVISO] No encontre el ancla en 10_Reportes.py; los PDFs estan en outputs/")
else:
    print("[INFO] Botones ya existian en Reportes")

print("\nAbre los 2 PDF de outputs/ para revisarlos")