"""Reescribe executive_report.py (version limpia, 7 secciones, sin errores)."""
...
def build_executive_pdf(df: pd.DataFrame) -> bytes:
    ...
    d1 = [["Indicador", "Valor", "Var. vs ano anterior"]] + \
         [[x["label"], x["value"], x["delta"] or "-"] for x in s["kpis"]]
    ...
    d4b = [["Cultivo", "CAGR"]] + \
          [[r["cultivo"], f"+{r['cagr']:.1f}%"] for _, r in s["crecen"].iterrows()] + \
          [[r["cultivo"], f"{r['cagr']:.1f}%"] for _, r in s["declinan"].iterrows()]
...
print("[OK] core/reports/executive_report.py (version limpia)")