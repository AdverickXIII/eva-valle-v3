"""Crea ui/pages/11_Comparador.py y la registra."""
from pathlib import Path

PAGE = '''"""Pagina 11: Comparador de municipios."""
...
st.set_page_config(page_title="Comparador | EVA Valle", icon="\\u2696\\uFE0F", layout="wide")
...
def main() -> None:
    ...
    ka, kb = st.columns(2)
    with ka:
        st.markdown(f"### \\U0001F170\\uFE0F {a}")
        for k, v in sa.items():
            st.markdown(f"- **{k}:** {v:,.0f}" if isinstance(v, float) else f"- **{k}:** {v}")
    with kb:
        st.markdown(f"### \\U0001F171\\uFE0F {b}")
        for k, v in sb.items():
            st.markdown(f"- **{k}:** {v:,.0f}" if isinstance(v, float) else f"- **{k}:** {v}")
...
if __name__ == "__main__":
    Path("ui/pages/11_Comparador.py").write_text(PAGE, encoding="utf-8")
    print("[OK] ui/pages/11_Comparador.py")

    app = Path("app.py")
    c = app.read_text(encoding="utf-8")
    anchor = 'st.Page("ui/pages/10_Reportes.py"'
    nueva = '    st.Page("ui/pages/11_Comparador.py", title="Comparador", icon="\\u2696\\uFE0F"),\n'
    if anchor in c and "11_Comparador.py" not in c:
        i = c.find(anchor)
        fin = c.find("\n", i)
        c = c[: fin + 1] + nueva + c[fin + 1 :]
        app.write_text(c, encoding="utf-8")
        print("[OK] app.py (pagina Comparador)")
