"""Reescribe executive_report.py: PDF ejecutivo 7 secciones con analisis dual."""
from pathlib import Path

REPORT = '''"""PDF ejecutivo de alto nivel (7 secciones) con firma y analisis dual."""
...
def build_executive_pdf(df: pd.DataFrame) -> bytes:
    ...
    d1 = [["Indicador", "Valor", "Var. vs ano anterior"]] + \
         [[x["label"], x["value"], x["delta"] or "-"] for x in k]
    story.append(KeepTogether([Paragraph("1. Panorama general", st_["Heading2"]),
    ...
    story.append(KeepTogether([
        Paragraph("2. Concentracion productiva: con cana vs sin cana", st_["Heading2"]),
    ...
    d3 = [["Indicador", "Valor"],
          ...
          ["Municipios Lider / Intermedio / Rezagado",
           f"{tier.get('Lider',0)} / {tier.get('Intermedio',0)} / {tier.get('Rezagado',0)}"]]
    story.append(KeepTogether([Paragraph("3. Distribucion territorial", st_["Heading2"]),
                               _tabla(d3), Spacer(1, 0.4 * cm)]))

    # 4. Tendencias y dinamica
    d4 = [["Ano", "Produccion (t)", "Rendimiento (t/ha)"]] + \
         [[str(int(r["ano"])), f"{r['produccion']:,.0f}", f"{r['rendimiento']:.2f}"]
          for _, r in s["tendencia"].iterrows()]
    d4b = [["Crecen (CAGR)", ""]] + [[r["cultivo"], f"+{r['cagr']:.1f}%"]
          for _, r in s["crecen"].iterrows()] + \
          [["Declinan (CAGR)", ""]] + [[r["cultivo"], f"{r['cagr']:.1f}%"]
          for _, r in s["declinan"].iterrows()]
    story.append(KeepTogether([Paragraph("4. Tendencias y dinamica (2019-2024)",
                               st_["Heading2"]), _tabla(d4), Spacer(1, 0.2 * cm),
                               _tabla(d4b), Spacer(1, 0.4 * cm)]))
    ...
    story.append(KeepTogether([Paragraph("5. Calidad y confiabilidad del dato",
                               st_["Heading2"]), _tabla(d5), Spacer(1, 0.4 * cm)]))

    # 6. Hallazgos
    h = [Paragraph(f"- {m}", st_["Normal"]) for m in s["mensajes"]]
    story.append(KeepTogether([Paragraph("6. Hallazgos clave", st_["Heading2")] + h +
                               [Spacer(1, 0.4 * cm)]))

    # 7. Recomendaciones
    r = [Paragraph(f"<b>{t}.</b> {d}", st_["Normal"]) for t, d in recomendaciones(df)]
    story.append(KeepTogether([Paragraph("7. Recomendaciones", st_["Heading2"]) + r +
                               [Spacer(1, 0.5 * cm)]))
...
print("[OK] core/reports/executive_report.py (PDF 7 secciones + dual)")
print("\nRecarga Streamlit (Ctrl+R) y descarga el Resumen Ejecutivo (PDF).")