"""Indice de Riesgo: 42 municipios en pantalla + PDF exportable."""
from pathlib import Path

PDFMOD = '''"""PDF del Indice de Riesgo Territorial (42 municipios)."""
import io
...
Path("core/reports/riesgo_report.py").write_text(PDFMOD, encoding="utf-8")
print("[OK] core/reports/riesgo_report.py creado")

p = Path("ui/pages/12_Alertas.py")
c = p.read_text(encoding="utf-8")

# import del PDF
if "riesgo_report" not in c:
    c = c.replace("from core.analytics.alerts import (generate_alerts, indice_riesgo_municipal)",
...
old_blk = '''    # ---------- INDICE DE RIESGO TERRITORIAL (estructural, sin filtros) ----------
    st.markdown("#### 🗺️ Indice de Riesgo Territorial (Top 15)")
    df_ir = indice_riesgo_municipal(df).head(15)'''
new_blk = '''    # ---------- INDICE DE RIESGO TERRITORIAL (42 municipios, sin filtros) ----------
    st.markdown("#### 🗺️ Indice de Riesgo Territorial (42 municipios)")
    df_ir = indice_riesgo_municipal(df)'''
if old_blk in c:
    c = c.replace(old_blk, new_blk, 1)
    print("[OK] Indice con los 42 municipios")

old_h = "fig.update_layout(height=520, xaxis=dict(range=[0, 100],"
new_h = "fig.update_layout(height=1250, xaxis=dict(range=[0, 100],"
if old_h in c:
    c = c.replace(old_h, new_h, 1)
    print("[OK] Altura ajustada para 42 barras")

old_cap = '''    st.caption("Indice compuesto = dependencia de un cultivo + baja diversidad + "
...
if old_cap in c:
    ...
    print("[OK] Boton de descarga PDF agregado")
else:
    p.write_text(c, encoding="utf-8")
    print("[AVISO] Caption distinto; revisa manualmente")
