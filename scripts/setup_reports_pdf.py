...
def _production_chart(f: dict) -> Drawing:
    """Barras: historico (verde) + proyeccion (naranja)."""
    years = f["years"]
...
def build_municipality_pdf(df: pd.DataFrame, municipio: str) -> bytes:
    ...
    if f:
        ...
        story.append(Paragraph(f"4. Proyeccion de produccion {fy}", st_["Heading2"]))
    ...
    story.append(Paragraph(
        ...
        "Proyeccion basada en tendencia lineal historica. "
