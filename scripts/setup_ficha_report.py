"""Crea core/reports/crop_report.py: export firmado por cultivo."""
...
def build_crop_pdf(df: pd.DataFrame, cultivo: str) -> bytes:
    ...
    story.append(Paragraph("1. Indicadores del cultivo", st_["Heading2"]))
    k = crop_kpis(df_c, df)
    kdata = [["Indicador", "Valor"]] + [[str(a), str(b)] for a, b in k.items()]
    t = Table(kdata, hAlign="LEFT"); t.setStyle(_style()); story.append(t)
    story.append(Spacer(1, 0.4 * cm))

    story.append(Paragraph("2. Concentracion territorial (Gini/HHI municipal)",
                           st_["Heading2"]))
    conc = crop_concentration(df_c)
    ...
    story.append(Paragraph("3. Historico anual", st_["Heading2"]))
    ...
    t3 = Table(ydata, hAlign="LEFT"); t3.setStyle(_style()); story.append(t3)
    story.append(Spacer(1, 0.4 * cm))

    story.append(Paragraph("4. Principales municipios", st_["Heading2"]))
    ...
    for _, r in crop_top_municipios(df_c).iterrows():
        mdata.append([r["municipio"], f"{r['produccion_t']:,.0f}",
                      f"{r['share_pct']:.1f}%"])
    t4 = Table(mdata, hAlign="LEFT"); t4.setStyle(_style()); story.append(t4)
...
print("[OK] core/reports/crop_report.py")
print("Sigue: python scripts\\setup_ficha_page.py")