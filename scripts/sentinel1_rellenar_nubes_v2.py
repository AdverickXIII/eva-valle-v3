"""Version 2: Sentinel-1 con filtros corregidos."""
...
for i, (_, row) in enumerate(faltantes.iterrows(), 1):
    ...
    try:
        # FIX: usar listContains para propiedades de lista
        ...
        vh_db = 10 * math.log10(st["VH"]) if st.get("VH") and st["VH"] > 0 else None
        rvi_v = st.get("RVI")
...
df_radar.to_csv("outputs/sentinel_radar_complemento_v2.csv", index=False)
