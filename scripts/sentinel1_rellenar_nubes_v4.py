"""Version 4 corregida: Sentinel-1 en dB (S1_GRD ya viene en decibelios)."""
import sys
import time
...
print(f"Registros nublados a rellenar: {len(faltantes)}\n")
...
for i, (_, row) in enumerate(faltantes.iterrows(), 1):
    ...
    try:
        ...
        n = s1.size().getInfo()
        if n == 0:
            radar.append({"municipio": m, "ano": ano, "vv_db": None,
                          "vh_db": None, "n_img_radar": 0})
            print(f"[{i}/{len(faltantes)}] {m} {ano}: sin imagenes radar")
            continue

        # S1_GRD ya viene en dB: usar VV y VH directamente
        comp = s1.median()
        st = comp.select(["VV", "VH"]).reduceRegion(
            reducer=ee.Reducer.mean(), geometry=zona,
            scale=20, maxPixels=1e9).getInfo()

        vv_db = st.get("VV")
        vh_db = st.get("VH")
        radar.append({"municipio": m, "ano": ano,
                      "vv_db": round(vv_db, 2) if vv_db is not None else None,
                      "vh_db": round(vh_db, 2) if vh_db is not None else None,
                      "n_img_radar": n})
        vv_s = f"{vv_db:.1f}" if vv_db is not None else "N/A"
        vh_s = f"{vh_db:.1f}" if vh_db is not None else "N/A"
        print(f"[{i}/{len(faltantes)}] {m} {ano}: VV={vv_s} dB VH={vh_s} dB ({n} img)")
    except Exception as e:
        print(f"[{i}/{len(faltantes)}] {m} {ano}: ERROR {str(e)[:80]}")
        radar.append({"municipio": m, "ano": ano, "vv_db": None, "vh_db": None,
                      "n_img_radar": 0})
    time.sleep(0.3)

df_radar = pd.DataFrame(radar)
df_radar.to_csv("outputs/sentinel_radar_complemento_v4.csv", index=False)

# --- Merge + coherencia con umbrales dB realistas ---
...
def coherencia_final(row):
    ...
    if vh is not None and not pd.isna(vh):
        ...
        if -22 <= vh <= -12 and area > 1000:
            return "✅ Coherente (radar)", "Radar"
        if vh < -24 and area > 10000:
...
print(f"\n{'='*60}")
print("COBERTURA FINAL OPTICO + RADAR")
print(f"{'='*60}")
print(df["fuente"].value_counts())
print("\nCoherencia final:")
print(df["coherencia_final"].value_counts())
tot = (df["fuente"] != "Ninguna").sum() / len(df) * 100
print(f"\nCobertura total: {tot:.1f}%")