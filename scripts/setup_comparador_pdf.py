"""Agrega PDF del cara a cara municipal (tabla + radar + mariposa)."""
from pathlib import Path

MOD = '''"""PDF de comparativa municipal: tabla con ganador + radar + mariposa."""
import io
...
Path("core/reports/comparador_pdf.py").write_text(MOD, encoding="utf-8")
print("[OK] core/reports/comparador_pdf.py creado")

# ---------- Pagina: boton PDF junto al CSV ----------
p = Path("ui/pages/11_Comparador.py")
c = p.read_text(encoding="utf-8")

if "comparador_pdf" not in c:
    anchor = "from ui.components.loading_states import render_empty_state"
    c = c.replace(anchor, anchor + "\nfrom core.reports.comparador_pdf import build_comparador_pdf")

old_btn = '''    safe = lambda s: "".join(ch for ch in s if ch.isalnum() or ch in "_-").lower()
    st.download_button("⬇️ Descargar comparativa (CSV)",
                       ...
                       mime="text/csv")'''
new_btn = '''    safe = lambda s: "".join(ch for ch in s if ch.isalnum() or ch in "_-").lower()
    d1, d2 = st.columns(2)
    with d1:
        st.download_button("⬇️ Descargar comparativa (CSV)",
                           data=comp_df.to_csv(index=False).encode("utf-8"),
...
if old_btn in c:
    ...
    print("[OK] Boton PDF agregado al Comparador")
else:
    p.write_text(c, encoding="utf-8")
    print("[AVISO] Bloque de botones distinto; revisa manualmente")

print("Reinicia Streamlit y descarga el PDF de Palmira vs Dagua")